...
public class HashTable<K, V> {
  ...
  public void put(K key, V value) {
    ...
    for (int i = index + 1, j = 0; j < table.length; i++, j++) {
      if (table[i % table.length] == null || table[i % table.length].key() == key) {
        table[i % table.length] = new Item(key, value);
        return;
      }
    }
  }
   ...
   */
  private void rehash() {
    if (size / table.length < .5) {
      return;
    }
    Item[] temp = (Item[]) new Item[table.length * 2];
  ...
  private int findKey(K key) {
    int index = indexFor(key.hashCode(), table.length);
    for (int i = index, j = 0; j < table.length; i++, j++) {
      if (table[index % table.length] != null && table[index % table.length].key() == key) {
        return i;
      }
    }
    return -1;
   ...
   * 
   * YOU SHOULD NOT CALL THIS METHOD DIRECTLY. IT IS A HELPER FOR THE indexFor METHOD ABOVE.
   */
