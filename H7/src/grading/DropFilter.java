...
public class DropFilter implements Filter
{
	...
	/**
	 * Default Constructor (i.e., construct a DropRule that drops the highest 
	 * and lowest Grade objects).
	 */
	public DropFilter()
	{
		this(true, true);        
	 ...
	 *
	 * @param shouldDropLowest  true if this Rule should drop the lowest Grade
	 * @param shouldDropHighest true if this Rule should drop the highest Grade
	 */
	 ...
	 * 
	 * This method throws a SizeException if the given List is null or the number of elements
	 * to be dropped is greater than or equal to the number of elements in the List.
	 *
	 * @param grades The original List of Grade objects
	 * @return        The List of resulting Grade objects (not necessarily in the same order)
	 * @throws SizeException if the List is null or too small
	 */
	public List<Grade> apply(final List<Grade> grades) throws SizeException
	{
		...
		// Sort the copy using the Grade object's compareTo() method
		Collections.sort(result); // Java v8: result.sort(null); 

		// Drop elements as appropriate
		if (shouldDropLowest)  result.remove(0);
		if (shouldDropHighest) result.remove(result.size()-1);
	...
	private int numberToDrop()
	{
		...
		if (shouldDropLowest)  ++result;
		if (shouldDropHighest) ++result;
